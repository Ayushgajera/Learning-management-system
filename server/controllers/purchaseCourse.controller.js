...
export const createPurchase = async (req, res) => {
    ...
    if (!process.env.RAZORPAY_KEY_ID || !process.env.RAZORPAY_SECRET) {
        console.error("❌ Critical: Razorpay API keys are missing from environment variables.");
        return res.status(500).json({ error: "Server configuration error: Payment gateway credentials missing." });
    }
