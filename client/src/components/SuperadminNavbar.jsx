...
import { FiHome, FiUser, FiUsers, FiSettings, FiLogOut, FiBell, FiMoon, FiSun, FiBarChart2, FiShield, FiBook, FiVideo } from 'react-icons/fi';
...
const superAdminMenuItems = [
  ...
  {
    to: "/superadmin/roles",
    icon: <FiShield className="w-4 h-4 text-slate-500 dark:text-slate-300 group-hover:text-indigo-500" />,
    label: "Manage Roles",
  },
  {
    to: "/superadmin/settings",
    icon: <FiSettings className="w-4 h-4 text-slate-500 dark:text-slate-300 group-hover:text-indigo-500" />,
    label: "Settings",
  },
];
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            >
              ...
              <div className="hidden lg:flex flex-col items-start">
                ...
                <span className="text-xs font-medium text-indigo-600 dark:text-indigo-400 capitalize">{userData?.role || 'Super Admin'}</span>
              </div>
            </button>
            <AnimatePresence>
        ...
        <div className="mx-auto flex h-16 w-full max-w-[1380px] items-center justify-between gap-4">
          ...
          <div className="flex items-center">
            ...
            <div className="hidden md:flex items-center ml-8 space-x-6 text-slate-700 dark:text-slate-200">
              {superAdminMenuItems.map(({ to, label, icon }) => (
                <RouterNavLink
                  key={to}
                  to={to}
                  className={({ isActive }) => `
          ...
          >
            <div className="px-4 py-4 space-y-4">
              ...
              <div className="space-y-2">
                ...
                <button
                  onClick={handleLogout}
                  className="w-full flex items-center gap-3 px-4 py-2 text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 hover:bg-red-50 dark:hover:bg-red-900 rounded-lg"
                >
